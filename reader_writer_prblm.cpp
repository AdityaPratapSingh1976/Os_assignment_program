...
using namespace std;
typedef struct {
    int count; // Represents the number of available resources or permits
} sem_t;
...
// Decrement (lock) the semaphore count and block if count is zero
int sem_wait(sem_t *sem) {
    if (sem == nullptr) {
        return -1; // Error: Invalid arguments
    }
    while (sem->count <= 0) {
        // Block until count becomes positive
        usleep(1000); // Sleep for 1 millisecond (for simplicity)
    }
    sem->count--;
    return 0; // Success
}

// Increment the semaphore count to release the semaphore
